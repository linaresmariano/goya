function CalendarCtrl($scope, $http, $q, CourseSchedule, SemesterTeacher, Semester, SemesterClassRoom, subjectService) {
  ...
  /* Eventos para agregar a la grilla*/
  $scope.events = []
  ...
  /***********************FUNCIONES PARA MANEJAR LOS EVENTOS DE LA GRILLA****************************/
  ...
  // Cuando se ajusta el horario de fin de un evento
  ...
  // Asignarle aula al curso
  function assignedClassRoom(classroom, event) {
    sendData({
      url: "/schedule/assignedClassRoom",
      data: {
        idClassRoom: classroom.id,
        idCourseSchedule: event.schedule.id,
  ...
  $scope.remove = function(index) {
    sendData({
      ...
      data: {
        idCourseSchedule: $scope.scheduleShow.schedule.id
      },
      success: function(data) {
        $scope.removeSchedule($scope.scheduleShow.schedule)
  ...
  $scope.hideTeacher = function(teacher) {
    if (!teacher.existTeacher($scope.scheduleShow.schedule.getNoVisibleTeachers())) {
      sendData({
        ...
        data: {
          idPatch: $scope.scheduleShow.schedule.patch.id,
  ...
  $scope.updatePatch = function() {
    sendData({
      url: "/patch/update",
      data: {
        extraHour: $scope.newPatchExtras.extraHour,
        extraDuration: $scope.newPatchExtras.extraDuration,
        idPatch: $scope.scheduleShow.schedule.patch.id
      },
      success: function(data) {
  ...
  $scope.hideSchedule = function() {
    sendData({
      ...
      data: {
        idPatch: $scope.scheduleShow.schedule.patch.id,
        visibility: !$scope.scheduleShow.schedule.patch.visibility
      },
      success: function(data) {
  ...
  function checkAmountTeachers(schedule, element, message) {
    if (schedule.getTeachers().length == 0) {
      ...
      messages[schedule.id] += '* Este curso no tiene profesores\n'
      markError(element, schedule)
      return typeMessage.danger

    } else if (schedule.semesterTeachers.length == 0) {

      messages[schedule.id] += '* Este horario no tiene profesores\n'
  ...
  function checkAmountEnrolled(schedule, element) {
    if (amountEnrolled(schedule.courses) < 5) {
      ...
      messages[schedule.id] += '* La cantidad de inscriptos es menor a 5 \n'
      markError(element, schedule)
      return typeMessage.danger

    } else if (amountEnrolled(schedule.courses) < 15) {

      messages[schedule.id] += '* La cantidad de inscriptos es menor a 15 \n'
      markWarning(element, schedule)
      return typeMessage.warning
  ...
  function checkPercentageEnrolled(schedule, element) {
    ...
    if (courseCapacity < (classroomCapacity / 2)) {
      messages[schedule.id] += '* La cantidad de cupos (' + courseCapacity + ') es mucho menor a la capacidad del aula (' + classroomCapacity + ') \n'
      markWarning(element, schedule)
      return typeMessage.warning
