app.factory('CourseSchedule', ['$http', 'SemesterTeacher', 'Patch', 'SemesterClassRoom', function($http, SemesterTeacher, Patch, SemesterClassRoom) {
  ...
  CourseSchedule.prototype = {
    ...
    getTitle: function() {
      return (this.getSubject() ? this.getSubject().nick : '' + "") + this.getCommissions() + "\n" + this.type + '\n Aula ' + (this.semesterClassRoom ? this.semesterClassRoom.classRoom.number : '??') + this.getNamesTeachers()
    },
    ...
    getStartAndEndTimes: function() {
      ...
      return {
        startHour: this.hour + extraHour,
        startMinutes: this.minutes + extraMinutes,
        endHour: this.hour + this.durationHour + extraHour + extraHourDuration,
        endMinutes: this.minutes + this.durationMinutes + extraMinutes + extraMinutesDuration
      }
    }
  };
